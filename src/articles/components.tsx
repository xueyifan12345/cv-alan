...
export function ArticleFooter({ lang, utmCampaign }: ArticleFooterProps) {
  ...
  const fellowUrl = `https://maven.com/marily-nika/ai-pm-bootcamp?utm_source=xueyifan&utm_medium=casestudy&utm_campaign=${utmCampaign}`
  return (
    <footer className="mt-16 pt-8 border-t border-border">
      <div className="flex items-start gap-3 mb-6">
        <img
          src="/foto-avatar-sm.webp"
          alt="Yifan Fernández de Valderrama"
          className="w-12 h-12 rounded-full shrink-0"
          width={48}
          height={48}
        />
        <div>
          <p className="font-medium text-foreground">Yifan Fernández de Valderrama</p>
      ...
      <Link
        to={lang === 'zh' ? '/sobre-mi' : '/about'}
        className="inline-block text-sm text-primary hover:underline transition-colors mb-6"
      >
        {lang === 'zh' ? 'Más sobre el autor →' : 'More about the author →'}
      </Link>
      <div className="flex gap-3 mb-8">
        <a href="https://linkedin.com/in/xueyifan" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#0A66C2]/10 border border-[#0A66C2]/20 text-sm font-medium text-[#0A66C2] hover:bg-[#0A66C2]/20 transition-colors">
        ...
        <a href="https://github.com/xueyifan" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-card border border-border text-sm font-medium text-muted-foreground hover:text-foreground hover:border-primary/30 transition-colors">
      ...
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span>&copy; {new Date().getFullYear()} Yifan Fernández de Valderrama. {f.copyright}</span>
